...
from pybaseball import batting_stats, statcast_batter, pitching_stats
...
# ── Step 1: Fetch today's MLB games and probable starters ─────────────────────
...
# ── Step 2: Fetch batter season stats ─────────────────────────────────────────
def get_batter_stats():
    year = datetime.now().year
    stats = batting_stats(year, qual=100)
    stats = stats.rename(columns={"Name": "name", "AVG": "avg", "OBP": "obp",
                                   "SLG": "slg", "BB%": "bb_pct", "K%": "k_pct"})
    stats["score"] = (
        stats["avg"].fillna(0) * 40 +
        stats["obp"].fillna(0) * 30 +
        stats["slg"].fillna(0) * 20 +
        (1 - stats["k_pct"].fillna(0.25)) * 10
    )
    return stats[["name", "avg", "obp", "slg", "score"]].sort_values("score", ascending=False)

# ── Step 3: Score and rank top hitters ────────────────────────────────────────
def get_top_picks(batter_stats, games, top_n=10):
    picks = batter_stats.head(40).copy()
    picks["confidence"] = (picks["score"] / picks["score"].max() * 100).round(1)
    picks["matchup"] = "vs. today's starter"
...
def build_email(picks, games):
    ...
    for i, (_, p) in enumerate(picks.iterrows(), 1):
        conf = p["confidence"]
        color = "#0F6E56" if conf >= 80 else "#BA7517" if conf >= 70 else "#666"
        ...
        <tr style="border-bottom:1px solid #f0f0f0;">
          ...
          <td style="padding:10px 8px;">
            ...
            <div style="font-size:11px;color:#888;margin-top:2px;">
              AVG {p['avg']:.3f} · OBP {p['obp']:.3f} · SLG {p['slg']:.3f}
            </div>
          </td>
    ...
    <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;background:#fff;">
      ...
      <div style="background:#f8f8f8;padding:16px 24px;text-align:center;">
        <p style="font-size:11px;color:#aaa;margin:0;">
          BTS Edge · Automated daily at 8am ET · Powered by MLB Stats API + FanGraphs
        </p>
      </div>
    </div>"""
    return html
...
def main():
    ...
    picks = get_top_picks(batter_stats, games)
